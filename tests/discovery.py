...
with tempfile.TemporaryDirectory(prefix='fund-mdns-') as tmp:
    ...
    p=subprocess.Popen([str(root/'build/fund-funeral'),'--data-dir',str(path/'vault'),'--test-listen',str(invite)],env={**os.environ,'QT_QPA_PLATFORM':'offscreen'},stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    try:
        for _ in range(100):
