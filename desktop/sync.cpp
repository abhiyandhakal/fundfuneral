...
SyncServer::SyncServer(Engine *e) : QTcpServer(e), engine(e) {
  ...
  if (!QFile::exists(certPath) || !QFile::exists(keyPath)) {
    ...
    if (!p.waitForFinished(30000) || p.exitCode() != 0)
      throw std::runtime_error(
          "Cannot generate device identity; install openssl");
  ...
  if (!cf.open(QIODevice::ReadOnly) || !kf.open(QIODevice::ReadOnly))
    throw std::runtime_error("Cannot read device identity");
  certificate = QSslCertificate(cf.readAll());
  ...
  connect(&expiry, &QTimer::timeout, this, &SyncServer::stop);
}
QString SyncServer::fingerprint() const {
...
QString SyncServer::start(bool pairing) {
  ...
  QJsonObject invite{{"format", "fund-funeral-pair"},
                     {"version", 1},
                     {"host", host},
                     {"port", serverPort()},
                     {"pin", fingerprint()},
...
void SyncServer::incomingConnection(qintptr descriptor) {
  ...
  s->setPrivateKey(key);
  s->setProtocol(QSsl::TlsV1_2OrLater);
  s->setPeerVerifyMode(QSslSocket::QueryPeer);
  // A self-signed peer is accepted only after its exact certificate is pinned
  // below.
  connect(s, &QSslSocket::sslErrors, s, [s](const QList<QSslError> &errors) {
  ...
  connect(s, &QSslSocket::readyRead, this, [this, s] {
    QByteArray b = s->property("buffer").toByteArray() + s->readAll();
    ...
    QJsonParseError err;
    auto doc = QJsonDocument::fromJson(b.left(line), &err);
    if (err.error != QJsonParseError::NoError || !doc.isObject()) {
      s->abort();
      return;
    }
    QString pin = QString::fromLatin1(
        s->peerCertificate().digest(QCryptographicHash::Sha256).toHex());
    try {
      if (s->peerCertificate().isNull())
        throw std::runtime_error("Device certificate required");
      process(s, doc.object(), pin);
    } catch (const std::exception &e) {
      s->write(
          QJsonDocument(QJsonObject{{"error", QString::fromUtf8(e.what())}})
              .toJson(QJsonDocument::Compact) +
          '\n');
      s->disconnectFromHost();
      emit status("Sync failed: " + QString::fromUtf8(e.what()));
    }
  });
  s->startServerEncryption();
}
void SyncServer::process(QSslSocket *s, const QJsonObject &r,
                         const QString &pin) {
  auto peers =
      QJsonDocument::fromJson(engine->setting("peers", "{}").toUtf8()).object();
  bool pairing = r["action"] == "pair";
  if (pairing) {
    if (secret.isEmpty() || r["secret"].toString() != secret)
      throw std::runtime_error("Pairing invitation expired or invalid");
  ...
  if (pairing) {
    ...
    engine->save(engine->query("merge", {r["events"]}).toObject());
  }
  auto outgoing = engine->query("changes", {r["clock"].toObject()});
  QJsonObject response{{"vault", engine->state()["vault"]},
                       {"events", outgoing},
  ...
  if (bytes.size() > 16 * 1024 * 1024)
    throw std::runtime_error("Sync batch exceeds 16 MiB; use backup transfer");
  s->write(bytes);
  s->disconnectFromHost();
  engine->setSetting("lastSync",
                     QDateTime::currentDateTime().toString(Qt::ISODate));
  emit status(pairing ? "Phone paired. Your records have been sent."
                      : "Sync complete. Both devices have exchanged changes.");
